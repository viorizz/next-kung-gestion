...
export function PDFViewer({
  ...
  useEffect(() => {
    if (manufacturer && productType) {
      ...
      const formattedManufacturer = manufacturer.toUpperCase().replace(/\s+/g, '-');
      const formattedProductType = productType.toUpperCase().replace(/\s+/g, '-');
  ...
  useEffect(() => {
    const loadPDF = async () => {
      ...
      try {
        ...
        setError('Failed to load PDF file. Please ensure the form template exists for this product type.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="flex flex-col items-center">
      ...
      {isReadOnly && (
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-yellow-800 text-sm">
          This order list has been submitted and cannot be modified. You can still export or print the PDF.
        </div>
      )}
    </div>
  );
